  ...
  setup() {
    ...
    function startGameInterval() {
      ...
      interval = setInterval(() => {
        ...
        setMole(
          getRandomInt(0, state.moleGrid.length),
          getRandomInt(0, state.moleGrid[0].length),
          1,
        );
  ...
  render() {
    ...
    return `
      <div class="game-container">
        ${
          state.hasWon
            ? `<h2>YOU WON LEVEL ${state.level} in ${getTime()}s</h2>`
            : `<h2>Whac a Mole (Level ${state.level})</h2>`
        }
        ...
        ${
          !state.hasWon
            ...
            ${state.moleGrid.each((dem1, x) =>
              dem1.each(
                (dem2, y) =>
                  `<div id="circle" data-x="${x}" data-y="${y}" class="${
                    dem2 === 1 ? 'one' : 'zero'
                  }"></div>`,
              ),
            )}
           </div>`
            : `<p class="won-text"><button id="next">Challenge with level ${
                state.level + 1
              }</button></p>`
        }
        <div class="source">
